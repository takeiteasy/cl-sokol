...
(defpackage :sokol-shape
  ...
  (:export
   ...
   sshape-color-4b
   sshape-color-4f
   sshape-color-vertex-attr-state
   sshape-cylinder
   sshape-cylinder-sizes
   sshape-element-range
   sshape-element-range
   sshape-index-buffer-desc
   sshape-mat4
   sshape-mat4
   sshape-mat4-transpose
   sshape-normal-vertex-attr-state
   sshape-plane
   sshape-plane-sizes
   sshape-position-vertex-attr-state
   sshape-range
   sshape-sizes
   sshape-sizes-item
   sshape-sphere
   sshape-sphere-sizes
   sshape-texcoord-vertex-attr-state
   sshape-torus
   sshape-torus-sizes
   sshape-vertex
   sshape-vertex-buffer-desc
   sshape-vertex-buffer-layout-state
   ))

(in-package :sokol-shape)
...
(cffi:defcstruct sshape-element-range
  ;; sshape_element_range_t
  (base-element :int)
  (num-elements :int)
)

(cffi:defcstruct sshape-sizes-item
  ;; sshape_sizes_item_t
  (num :uint32)
  (size :uint32)
)

(cffi:defcstruct sshape-sizes
  (vertices (:struct sshape-sizes-item))
  (indices (:struct sshape-sizes-item))
)

(cffi:defcstruct sshape-buffer-item
  ;; sshape_buffer_item_t
  (buffer (:struct sshape-range))
  (data-size :size)
  (shape-offset :size)
)

(cffi:defcstruct sshape-buffer
  (valid :bool)
  (vertices (:struct sshape-buffer-item))
  (indices (:struct sshape-buffer-item))
)

(cffi:defcstruct sshape-plane
  ;; sshape_plane_t
  (width :float)
  (depth :float)
  (tiles :uint16)
  ...
  (random-colors :bool)
  (merge :bool)
  (transform (:struct sshape-mat4))
)

(cffi:defcstruct sshape-box
  (width :float)
  (height :float)
  (depth :float)
  (tiles :uint16)
...
(cffi:defcstruct sshape-cylinder
  (radius :float)
  (height :float)
  (slices :uint16)
  (stacks :uint16)
...
(cffi:defcfun ("sshape_build_plane" sshape-build-plane) (:struct sshape-buffer)
  ;; sshape_build_plane
  (buf (:pointer (:struct sshape-buffer)))
  (params (:pointer (:struct sshape-plane)))
)

(cffi:defcfun ("sshape_build_box" sshape-build-box) (:struct sshape-buffer)
  (buf (:pointer (:struct sshape-buffer)))
  (params (:pointer (:struct sshape-box)))
)

(cffi:defcfun ("sshape_build_sphere" sshape-build-sphere) (:struct sshape-buffer)
  (buf (:pointer (:struct sshape-buffer)))
  (params (:pointer (:struct sshape-sphere)))
)

(cffi:defcfun ("sshape_build_cylinder" sshape-build-cylinder) (:struct sshape-buffer)
  (buf (:pointer (:struct sshape-buffer)))
  (params (:pointer (:struct sshape-cylinder)))
)

(cffi:defcfun ("sshape_build_torus" sshape-build-torus) (:struct sshape-buffer)
  (buf (:pointer (:struct sshape-buffer)))
  (params (:pointer (:struct sshape-torus)))
)

(cffi:defcfun ("sshape_plane_sizes" sshape-plane-sizes) (:struct sshape-sizes)
  ;; sshape_plane_sizes
  (tiles :uint32)
)

(cffi:defcfun ("sshape_box_sizes" sshape-box-sizes) (:struct sshape-sizes)
  (tiles :uint32)
)

(cffi:defcfun ("sshape_sphere_sizes" sshape-sphere-sizes) (:struct sshape-sizes)
  (slices :uint32)
  (stacks :uint32)
)

(cffi:defcfun ("sshape_cylinder_sizes" sshape-cylinder-sizes) (:struct sshape-sizes)
  (slices :uint32)
  (stacks :uint32)
)

(cffi:defcfun ("sshape_torus_sizes" sshape-torus-sizes) (:struct sshape-sizes)
  (sides :uint32)
  (rings :uint32)
)

(cffi:defcfun ("sshape_element_range" sshape-element-range) (:struct sshape-element-range)
  ;; sshape_element_range
  (buf (:pointer (:struct sshape-buffer)))
)

(cffi:defcfun ("sshape_vertex_buffer_desc" sshape-vertex-buffer-desc) (:struct sokol-gfx:sg-buffer-desc)
  (buf (:pointer (:struct sshape-buffer)))
)

(cffi:defcfun ("sshape_index_buffer_desc" sshape-index-buffer-desc) (:struct sokol-gfx:sg-buffer-desc)
  (buf (:pointer (:struct sshape-buffer)))
)

(cffi:defcfun ("sshape_vertex_buffer_layout_state" sshape-vertex-buffer-layout-state) (:struct sokol-gfx:sg-vertex-buffer-layout-state)
)

(cffi:defcfun ("sshape_position_vertex_attr_state" sshape-position-vertex-attr-state) (:struct sokol-gfx:sg-vertex-attr-state)
)

(cffi:defcfun ("sshape_normal_vertex_attr_state" sshape-normal-vertex-attr-state) (:struct sokol-gfx:sg-vertex-attr-state)
)

(cffi:defcfun ("sshape_texcoord_vertex_attr_state" sshape-texcoord-vertex-attr-state) (:struct sokol-gfx:sg-vertex-attr-state)
)

(cffi:defcfun ("sshape_color_vertex_attr_state" sshape-color-vertex-attr-state) (:struct sokol-gfx:sg-vertex-attr-state)
)

(cffi:defcfun ("sshape_color_4f" sshape-color-4f) :uint32
  ;; sshape_color_4f
  (r :float)
  (g :float)
  (b :float)
...
(cffi:defcfun ("sshape_color_3f" sshape-color-3f) :uint32
  (r :float)
  (g :float)
  (b :float)
)

(cffi:defcfun ("sshape_color_4b" sshape-color-4b) :uint32
  (r :uint8)
  (g :uint8)
  (b :uint8)
  (a :uint8)
...
(cffi:defcfun ("sshape_color_3b" sshape-color-3b) :uint32
  (r :uint8)
  (g :uint8)
  (b :uint8)
)

(cffi:defcfun ("sshape_mat4" sshape-mat4) (:struct sshape-mat4)
  ;; sshape_mat4
  (m (:pointer :float))
)

(cffi:defcfun ("sshape_mat4_transpose" sshape-mat4-transpose) (:struct sshape-mat4)
  (m (:pointer :float))
)
