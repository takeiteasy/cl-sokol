...
(cffi:defcfun ("stm_setup" stm-setup) :void
)

(cffi:defcfun ("stm_now" stm-now) :uint64
)

(cffi:defcfun ("stm_diff" stm-diff) :uint64
  (new-ticks :uint64)
  (old-ticks :uint64)
)

(cffi:defcfun ("stm_since" stm-since) :uint64
  (start-ticks :uint64)
)

(cffi:defcfun ("stm_laptime" stm-laptime) :uint64
  (last-time (:pointer :uint64))
)

(cffi:defcfun ("stm_round_to_common_refresh_rate" stm-round-to-common-refresh-rate) :uint64
  (frame-ticks :uint64)
)

(cffi:defcfun ("stm_sec" stm-sec) :double
  (ticks :uint64)
)

(cffi:defcfun ("stm_ms" stm-ms) :double
  (ticks :uint64)
)

(cffi:defcfun ("stm_us" stm-us) :double
  (ticks :uint64)
)

(cffi:defcfun ("stm_ns" stm-ns) :double
  (ticks :uint64)
)
