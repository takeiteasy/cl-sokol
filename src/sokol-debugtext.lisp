...
(defpackage :sokol-debugtext
  (:use :cl :cffi)
  (:nicknames :sdtx)
  (:export
   +SDTX-LOGITEM-ADD-COMMIT-LISTENER-FAILED+
   +SDTX-LOGITEM-CANNOT-DESTROY-DEFAULT-CONTEXT+
   +SDTX-LOGITEM-COMMAND-BUFFER-FULL+
   +SDTX-LOGITEM-CONTEXT-POOL-EXHAUSTED+
   +SDTX-LOGITEM-MALLOC-FAILED+
   +SDTX-LOGITEM-OK+
   sdtx-allocator
   sdtx-canvas
   sdtx-color1i
   ...
   sdtx-context-desc
   sdtx-context-draw
   sdtx-context-draw-layer
   sdtx-crlf
   sdtx-default-context
   sdtx-desc
   sdtx-destroy-context
   sdtx-draw
   sdtx-draw-layer
   sdtx-font
   sdtx-font-c64
...
(cffi:defcstruct sdtx-logger
  ;; sdtx_logger_t
  (func :pointer)
  (user-data :pointer)
)

(cffi:defcstruct sdtx-context
  ;; sdtx_context
  (id :uint32)
)

(cffi:defcstruct sdtx-range
  ;; sdtx_range
  (ptr :pointer)
  (size :size)
)

(cffi:defcstruct sdtx-font-desc
  (data (:struct sdtx-range))
  (first-char :uint8)
  (last-char :uint8)
)

(cffi:defcstruct sdtx-context-desc
  ;; sdtx_context_desc_t
  (max-commands :int)
  (char-buf-size :int)
  (canvas-width :float)
...
(cffi:defcstruct sdtx-allocator
  ;; sdtx_allocator_t
  (alloc-fn :pointer)
  (free-fn :pointer)
  (user-data :pointer)
...
(cffi:defcstruct sdtx-desc
  ;; sdtx_desc_t
  (context-pool-size :int)
  (printf-buf-size :int)
  (fonts (:struct sdtx-font-desc) :count 8)
  (context (:struct sdtx-context-desc))
  (allocator (:struct sdtx-allocator))
  (logger (:struct sdtx-logger))
)

;; Functions

(cffi:defcfun ("sdtx_setup" sdtx-setup) :void
  ;; sdtx_setup
  (desc (:pointer (:struct sdtx-desc)))
)

(cffi:defcfun ("sdtx_shutdown" sdtx-shutdown) :void
)

(cffi:defcfun ("sdtx_font_kc853" sdtx-font-kc853) (:struct sdtx-font-desc)
  ;; sdtx_font_kc853
)

(cffi:defcfun ("sdtx_font_kc854" sdtx-font-kc854) (:struct sdtx-font-desc)
)

(cffi:defcfun ("sdtx_font_z1013" sdtx-font-z1013) (:struct sdtx-font-desc)
)

(cffi:defcfun ("sdtx_font_cpc" sdtx-font-cpc) (:struct sdtx-font-desc)
)

(cffi:defcfun ("sdtx_font_c64" sdtx-font-c64) (:struct sdtx-font-desc)
)

(cffi:defcfun ("sdtx_font_oric" sdtx-font-oric) (:struct sdtx-font-desc)
)

(cffi:defcfun ("sdtx_make_context" sdtx-make-context) (:struct sdtx-context)
  ;; sdtx_make_context
  (desc (:pointer (:struct sdtx-context-desc)))
)

(cffi:defcfun ("sdtx_destroy_context" sdtx-destroy-context) :void
  (ctx (:struct sdtx-context))
)

(cffi:defcfun ("sdtx_set_context" sdtx-set-context) :void
  (ctx (:struct sdtx-context))
)

(cffi:defcfun ("sdtx_get_context" sdtx-get-context) (:struct sdtx-context)
)

(cffi:defcfun ("sdtx_default_context" sdtx-default-context) (:struct sdtx-context)
)

(cffi:defcfun ("sdtx_draw" sdtx-draw) :void
  ;; sdtx_draw
)

(cffi:defcfun ("sdtx_context_draw" sdtx-context-draw) :void
  (ctx (:struct sdtx-context))
)

(cffi:defcfun ("sdtx_draw_layer" sdtx-draw-layer) :void
  (layer-id :int)
)

(cffi:defcfun ("sdtx_context_draw_layer" sdtx-context-draw-layer) :void
  (ctx (:struct sdtx-context))
  (layer-id :int)
)

(cffi:defcfun ("sdtx_layer" sdtx-layer) :void
  ;; sdtx_layer
  (layer-id :int)
)

(cffi:defcfun ("sdtx_font" sdtx-font) :void
  ;; sdtx_font
  (font-index :int)
)

(cffi:defcfun ("sdtx_canvas" sdtx-canvas) :void
  ;; sdtx_canvas
  (w :float)
  (h :float)
)

(cffi:defcfun ("sdtx_origin" sdtx-origin) :void
  ;; sdtx_origin
  (x :float)
  (y :float)
)

(cffi:defcfun ("sdtx_home" sdtx-home) :void
  ;; sdtx_home
)

(cffi:defcfun ("sdtx_pos" sdtx-pos) :void
  (x :float)
  (y :float)
)

(cffi:defcfun ("sdtx_pos_x" sdtx-pos-x) :void
  (x :float)
)

(cffi:defcfun ("sdtx_pos_y" sdtx-pos-y) :void
  (y :float)
)

(cffi:defcfun ("sdtx_move" sdtx-move) :void
  (dx :float)
  (dy :float)
)

(cffi:defcfun ("sdtx_move_x" sdtx-move-x) :void
  (dx :float)
)

(cffi:defcfun ("sdtx_move_y" sdtx-move-y) :void
  (dy :float)
)

(cffi:defcfun ("sdtx_crlf" sdtx-crlf) :void
)

(cffi:defcfun ("sdtx_color3b" sdtx-color3b) :void
  ;; sdtx_color3b
  (r :uint8)
  (g :uint8)
  (b :uint8)
...
(cffi:defcfun ("sdtx_color3f" sdtx-color3f) :void
  (r :float)
  (g :float)
  (b :float)
)

(cffi:defcfun ("sdtx_color4b" sdtx-color4b) :void
  (r :uint8)
  (g :uint8)
  (b :uint8)
  (a :uint8)
...
(cffi:defcfun ("sdtx_color4f" sdtx-color4f) :void
  (r :float)
  (g :float)
  (b :float)
  (a :float)
...
(cffi:defcfun ("sdtx_color1i" sdtx-color1i) :void
  (rgba :uint32)
)

(cffi:defcfun ("sdtx_putc" sdtx-putc) :void
  ;; sdtx_putc
  (c :char)
)

(cffi:defcfun ("sdtx_puts" sdtx-puts) :void
  (str :string)
)

(cffi:defcfun ("sdtx_putr" sdtx-putr) :void
  (str :string)
  (len :int)
)

(cffi:defcfun ("sdtx_get_cleared_fmt_buffer" sdtx-get-cleared-fmt-buffer) (:struct sdtx-range)
