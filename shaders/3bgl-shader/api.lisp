...
(defun glsl-es-version-p (version)
  "Return T if VERSION is a GLSL ES version number.
GLSL ES versions: 100 (ES 2.0), 300 (ES 3.0), 310 (ES 3.1), 320 (ES 3.2).
Desktop GLSL versions: 110, 120, 130, 140, 150, 330, 400, 410, 420, 430, 440, 450, 460."
  (member version '(100 300 310 320)))

(defmethod generate-output (objects inferred-types (backend (eql :glsl))
                            &key version extensions &allow-other-keys)
  (let ((es-version-p (glsl-es-version-p version)))
    (with-output-to-string (*standard-output*)
      ;; Version pragma - add "es" suffix for ES versions
      (if es-version-p
          (format t "#version ~a es~%" version)
      ...
      ;; Precision qualifiers required for ES versions
      (when es-version-p
        (format t "precision highp float;~%")
        (format t "precision highp int;~%"))
      ;; Invariant gl_Position for vertex shaders (not supported in ES 2.0/version 100)
      (when (and (eql *current-shader-stage* :vertex)
                 (not (eql version 100)))
